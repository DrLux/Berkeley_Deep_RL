...
class ModelBasedRL(object):
    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            # Initial state and all actions are given
            pred_states.append(states[0])
            for i in range(len(actions)):
                pred_states.append(self._policy.predict(pred_states[i], actions[i]))
                #print("predicted vs official: ",pred_states[i][0], " vs ", states[i][0])
    ...
    def run_q2(self):
        ...
        while not done:
            ...
            #RANDOM POLICY action = utils.RandomPolicy(self._env).get_action(state)
            timeit.stop('get action')
            timeit.start('env step')
            next_state, reward, done, _ = self._env.step(action)
            timeit.stop('env step')
            done = done or (t >= self._max_rollout_length)
            eval_dataset.add(state, action, next_state, reward, done)
            state = next_state
